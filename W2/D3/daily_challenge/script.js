...
for (let instanceNumber = 0; instanceNumber < arr.length; ++instanceNumber) {
    for (let nextNumber = (instanceNumber + 1); nextNumber < arr.length; ++nextNumber) {
        if (arr[instanceNumber] < arr[nextNumber]) {
            let tmp = arr[instanceNumber]; // instancenr = 5; nextnr = 0 // tmp =5
